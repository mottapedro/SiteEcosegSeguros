# DES-007 — Home Narrative Design

**Marca:** EcoSeg Seguros

**Identificador:** DES-007
...
**Visual sugerido:** forma concêntrica (`DES-006 §3`) contendo uma fotografia documental (`DES-006 §8`) de um ambiente real de operação — nunca pessoa sorrindo, nunca ícone de cofre/escudo. O enquadramento é o núcleo; nada ao redor ainda (as camadas vêm no Capítulo 2).
...
**Motion:** entrada per `DES-005 §6` (Hero Animation) — eyebrow → headline → subheadline, `contour-out`, sequencial. Nenhum CTA para animar ainda neste capítulo.
...
**Conteúdo:** lista curta de adversidades reais e específicas (não genéricas) por segmento — reaproveita a lógica de `Segmentos` (`DES-002`), mas aqui como reconhecimento, não como navegação. Tom: nomear sem assustar (`BD-002 §5`: nunca "você pode perder tudo").

**Visual sugerido:** camadas visuais ao redor do núcleo do Capítulo 1 (`DES-006 §5`) — cada adversidade é uma camada distinta, não uma lista de ícones soltos. Curvas de contorno (`DES-006 §4`) como divisores entre esta seção e a anterior.
...
**Motion:** `DES-005 §7` (Scroll Reveal) — cada adversidade revelada em stagger (40–60ms), reforçando "leitura camada por camada", não simultânea.

---

## Capítulo 3 — A Compreensão

**Objetivo:** mostrar que as adversidades nomeadas no Capítulo 2 podem ser mapeadas e entendidas — a ponte metodológica antes de qualquer produto (`DES-001 §2`, "Mapa Estratégico do Contexto" → "Compreensão"). Corresponde à seção `Mapa Estratégico do Contexto` e `Metodologia` de `DES-002`, resumidas aqui como prévia.
...
**Conteúdo:** explicação breve do processo de diagnóstico (`BD-002 §6`: Ouvir → Compreender → Orientar → Recomendar), com link para a página completa de Metodologia/Mapa Estratégico do Contexto (`DES-002`) — a Home não precisa explicar o método inteiro, apenas provar que ele existe.

**Visual sugerido:** diagrama simplificado (`DES-006 §10`, Diagram Language) mostrando o núcleo (patrimônio) com linhas de interseção indicando "pontos mapeados" — nunca um ícone de lupa genérico (clichê de "busca" sem relação com a metáfora, já vetado em `DES-006 §6`).
...
**CTA:** primeiro CTA de exploração (não de conversão): "Entender o método" ou "Ver como mapeamos o contexto" → linka para `Metodologia`/`Mapa Estratégico do Contexto` (`DES-002`). Ainda não é "Solicitar diagnóstico".

**Motion:** `DES-005 §7`, mesma lógica de stagger; diagrama pode ter uma leve animação de "conexão se formando" entre núcleo e pontos de contexto ao entrar na viewport (uma única vez, `once: true`).

---

## Capítulo 4 — A Decisão

**Objetivo:** apresentar as soluções (`Soluções`, `DES-002`) como a decisão que decorre naturalmente da compreensão — nunca como catálogo de produtos apresentado cedo demais. Corresponde ao `Products` já implementado, mas só faz sentido narrativo _depois_ dos Capítulos 1–3, não antes.
...
**Visual sugerido:** já alinhado — cards com estrutura de núcleo (produto) + camadas de informação (riscos, público, exemplo), per `DES-006 §3`/`§5`.

**Componentes:** `Products` (existente, sem mudança estrutural — apenas reposicionamento na sequência da página).

**CTA:** "Entender [Produto]" (já implementado, `DES-003 §5`) — primeiro CTA que se aproxima de decisão, mas ainda de exploração, não de compromisso. Correto como está.

**Motion:** já parcialmente coberto por `DES-005 §7` (Scroll Reveal) quando implementado — nenhuma mudança de especificação necessária além do que já está em `DES-005`.
...
**Conteúdo:** depoimentos já implementados (`testimonials.tsx`) — manter, já alinhados a `DES-003` (tom calmo, sem exagero). `Partners` (seguradoras parceiras) permanece como evidência de solidez técnica (comparação entre seguradoras, `BD-002`), não como "selo de confiança" genérico.
...
**Motion:** `DES-005 §7`, stagger leve entre depoimentos.
...
**Conteúdo:** formulário de contato (`contact-form.tsx`) — já implementado e auditado em `DES-003 §10` (sem violações).

**Visual sugerido:** o formulário como o "ponto de entrada para uma nova camada de mapeamento" (o diagnóstico real, que começa depois do envio) — visualmente, pode usar a mesma lógica de contenção (`DES-006 §8`) para o card do formulário, já parcialmente presente (`border`, `rounded-md`).

**Componentes:** `Cta`, `Contact` (existentes, sem mudança estrutural).

**CTA:** "Agendar consultoria" (`Cta`) e "Enviar solicitação" (`Contact`) — já implementados e alinhados a `DES-003 §5`. Nenhuma mudança necessária.

**Motion:** nenhuma animação de "celebração" ao enviar (já corretamente evitado, `DES-003 §8`) — apenas a confirmação textual já implementada.
...
Depende de: `DES-001 §2` (Progress Protection Philosophy — fonte direta dos seis capítulos), `DES-002` (seções já definidas: Home, Segmentos, Soluções, Metodologia), `DES-006` (tratamento visual por capítulo), `DES-005` (motion por capítulo), `DES-003` (CTAs e tom).
